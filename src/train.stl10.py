...
X_train = read_all_images(path_to_data=get_file("stl10_binary/stl_Xtrain_128px.bin"), img_size=image_size)
X_test = read_all_images(path_to_data=get_file("stl10_binary/stl_Xtest_128px.bin"), img_size=image_size)
...
bn1, bn2, bn3, bn4, bn5, bn6, debn4, debn3, debn2, debn1, reconstruction_ori, reconstruction = model.build_reconstruction(
    images_tf, is_train)
...
loss_recon_overlap = tf.reduce_mean(
    tf.sqrt(1e-5 + tf.reduce_sum(loss_recon_ori * mask_overlap, [1, 2, 3]))) * 10.  # Loss for overlapping region
...
for epoch in range(n_epochs):
    ...
    for start, end in zip(
        ...
        if iters % 100 == 0:
            test_images_ori = X_test[:batch_size]
        ...
        _, loss_G_val, adv_pos_val, adv_neg_val, loss_recon_val, loss_adv_G_val, reconstruction_vals, recon_ori_vals, bn1_val, bn2_val, bn3_val, bn4_val, bn5_val, bn6_val, debn4_val, debn3_val, debn2_val, debn1_val = sess.run(
            [train_op_G, loss_G, adversarial_pos, adversarial_neg, loss_recon, loss_adv_G, reconstruction,
             reconstruction_ori, bn1, bn2, bn3, bn4, bn5, bn6, debn4, debn3, debn2, debn1],
            feed_dict={
                images_tf: images,
                images_hiding: crops,
